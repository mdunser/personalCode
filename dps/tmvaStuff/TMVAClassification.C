...
void TMVAClassification( std::string selectionName, std::string charge, TString myMethodList = "" ) 
{
   ...
   factory->AddVariable( "Mll"      , "Inv. mass of leptons", "", 'F');
   factory->AddVariable( "MT1"      , "m_{T}^{leading}", "", 'F');
   factory->AddVariable( "MET"      , "E_{T}^{miss}", "", 'F');
   factory->AddVariable( "Jet0Pt"   , "jet-p_{T}^{0}", "", 'F');
   factory->AddVariable( "eta1"     , "#eta_{leading}", "", 'F');
   factory->AddVariable( "NbJ"      , "#N_{b-jets}^{loose}", "", 'I');
   factory->AddVariable( "NbJmed"   , "#N_{b-jets}^{medium}", "", 'I');
   factory->AddVariable( "MT2"      , "#M_{T2}", "", 'F');
   factory->AddVariable( "dPhiMLs"  , "#dPhiMLs", "", 'I');
   factory->AddVariable( "NMus"      , "#NMus", "", 'F');
   factory->AddVariable( "PFIso1"    , "#iso1", "", 'F');
   factory->AddVariable( "PFIso2"    , "#iso2", "", 'F');
   ...
   if (ReadDataFromAsciiIFormat) {
      ...
      TFile* signalFile = TFile::Open("/shome/mdunser/finalRA5/CMSSW_5_3_7_patch5/src/DiLeptonAnalysis/NTupleProducer/macros/plots/Jun13_DPS/YieldsFiles/DPSWW_Yields.root");      
      TTree *signal     = (TTree*)signalFile->Get("SigEvents");

      TChain* background = new TChain("SigEvents");
      background->Add("/shome/mdunser/finalRA5/CMSSW_5_3_7_patch5/src/DiLeptonAnalysis/NTupleProducer/macros/plots/Jun13_DPS/YieldsFiles/WGstarMu_Yields.root");
      background->Add("/shome/mdunser/finalRA5/CMSSW_5_3_7_patch5/src/DiLeptonAnalysis/NTupleProducer/macros/plots/Jun13_DPS/YieldsFiles/W+W+_Yields.root");
      background->Add("/shome/mdunser/finalRA5/CMSSW_5_3_7_patch5/src/DiLeptonAnalysis/NTupleProducer/macros/plots/Jun13_DPS/YieldsFiles/W-W-_Yields.root");
      background->Add("/shome/mdunser/finalRA5/CMSSW_5_3_7_patch5/src/DiLeptonAnalysis/NTupleProducer/macros/plots/Jun13_DPS/YieldsFiles/TTbarW_Yields.root");
      background->Add("/shome/mdunser/finalRA5/CMSSW_5_3_7_patch5/src/DiLeptonAnalysis/NTupleProducer/macros/plots/Jun13_DPS/YieldsFiles/WZTo3LNu_Yields.root");
      background->Add("/shome/mdunser/finalRA5/CMSSW_5_3_7_patch5/src/DiLeptonAnalysis/NTupleProducer/macros/plots/Jun13_DPS/YieldsFiles/HWW_Yields.root");
      background->Add("/shome/mdunser/finalRA5/CMSSW_5_3_7_patch5/src/DiLeptonAnalysis/NTupleProducer/macros/plots/Jun13_DPS/YieldsFiles/TTJets_Yields.root");
   ...
   if( charge == "plus" ) {
     mycuts = "Charge== 1 && SystFlag==0 && pT1>20. && pT1>10. && TLCat==0 && Flavor==0 && PassZVeto==1 && Pass3rdVeto==1"; // for example: TCut mycuts = "abs(var1)<0.5 && abs(var2-0.5)<1";
     mycutb = "Charge== 1 && SystFlag==0 && pT1>20. && pT1>10. && TLCat==0 && Flavor==0 && PassZVeto==1 && Pass3rdVeto==1"; // for example: TCut mycutb = "abs(var1)<0.5";
   } else if( charge == "minus" ) {
     mycuts = "Charge==-1 && SystFlag==0 && pT1>20. && pT1>10. && TLCat==0 && Flavor==0 && PassZVeto==1 && Pass3rdVeto==1"; // for example: TCut mycuts = "abs(var1)<0.5 && abs(var2-0.5)<1";
     mycutb = "Charge==-1 && SystFlag==0 && pT1>20. && pT1>10. && TLCat==0 && Flavor==0 && PassZVeto==1 && Pass3rdVeto==1"; // for example: TCut mycutb = "abs(var1)<0.5";
   } else if( charge == "all" ) {
     mycuts = "              SystFlag==0 && pT1>20. && pT1>10. && TLCat==0 && Flavor==0 && PassZVeto==1 && Pass3rdVeto==1"; // for example: TCut mycuts = "abs(var1)<0.5 && abs(var2-0.5)<1";
     mycutb = "              SystFlag==0 && pT1>20. && pT1>10. && TLCat==0 && Flavor==0 && PassZVeto==1 && Pass3rdVeto==1"; // for example: TCut mycutb = "abs(var1)<0.5";
   } else {
   ...
   // // factory->AddVariable( "NbJmed"   , "#N_{b-jets}^{medium}", "", 'I');
   // // factory->AddVariable( "MT2"      , "#M_{T2}", "", 'F');
   // // factory->AddVariable( "dPhiMLs"      , "#dPhiMLs", "", 'F');
   // // factory->AddVariable( "NMus"      , "#NMus", "", 'I');
   // // factory->AddVariable( "PFIso1"    , "#iso1", "", 'F');
   // // factory->AddVariable( "PFIso2"    , "#iso2", "", 'F');
      bookConditions = "H:!V:FitMethod=MC";
      bookConditions += ":VarProp[0]=FMin";  //ht
      bookConditions += ":VarProp[1]=FSmart";//pt1
      bookConditions += ":VarProp[2]=FSmart";//pt2
      bookConditions += ":VarProp[3]=FMin";  //nj
      bookConditions += ":VarProp[4]=FMin";  //mll
      bookConditions += ":VarProp[5]=FMax";  //mt1
      bookConditions += ":VarProp[6]=FSmart";  //met
      bookConditions += ":VarProp[7]=FMin";  //jet0pt
      bookConditions += ":VarProp[8]=FSmart";  //eta1
      bookConditions += ":VarProp[9]=FMin";  //nbj
      bookConditions += ":VarProp[10]=FMin";  //nbjmed
      bookConditions += ":VarProp[11]=FSmart";  //mt2
      bookConditions += ":VarProp[12]=FMax";  //dphimls
      bookConditions += ":VarProp[13]=FMin";  //nmus
      bookConditions += ":VarProp[14]=FMin";  //nmus
      bookConditions += ":VarProp[15]=FMin";  //nmus
   ...
   if (Use["Cuts"]) {
    ...
    for( unsigned iEff=1; iEff<11; ++iEff ) {
      ...
      bool found_pT2    = false;
      // bool found_NJ     = false;
      // bool found_Jet0Pt = false;
      bool found_NbJ    = false;
      bool found_dPhiMLs= false;
       ...
       for( unsigned iCut=0; iCut<cutsMin.size(); ++iCut) {
         ...
         // marc if( varName=="TMath_Min_pT1,pT2_") {
         // marc   ofs << "pT1 " << cutsMin[iCut] << " " << cutsMax[iCut] << std::endl;
         // marc   ofs << "pT2 " << cutsMin[iCut] << " " << cutsMax[iCut] << std::endl;
         // marc   found_pT1 = true;
         // marc   found_pT2 = true;
         // marc } else {
         ...
         if( varName=="HT"  ) found_HT = true;
         if( varName=="pT1" ) found_pT1 = true;
         if( varName=="pT2" ) found_pT2 = true;
         // if( varName=="NJ" ) found_NJ = true;
         // if( varName=="Jet0Pt" ) found_Jet0Pt = true;
         if( varName=="NbJ" ) found_NbJ = true;
         if( varName=="dPhiMLs" ) found_dPhiMLs = true;
       }
       ...
       if( !found_HT  ) ofs << "HT  0. 100." << std::endl;
       if( !found_pT1 ) ofs << "pT1 10. 100000." << std::endl;
       if( !found_pT2 ) ofs << "pT2 10. 100000." << std::endl;
       // if( !found_Jet0Pt )  ofs << "Jet0Pt 1 100000." << std::endl;
       // if( !found_NJ )  ofs << "NJ 1 100000." << std::endl;
       if( !found_NbJ ) ofs << "NbJ 1 100000." << std::endl;
       if( !found_dPhiMLs ) ofs << "dPhiMLs 1 100000." << std::endl;
